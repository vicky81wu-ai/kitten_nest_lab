...
export class DoorAwayController {
  ...
  get status() {
    return {
      ...
      nailiReturnAnchor: this.calibration.nailiReturnAnchor,
  ...
  returnNailiThroughFrame() {
    ...
    this.navigateExact(naili, [this.calibration.point1],
      () => this.moveNailiToIndoorAnchor(), { facing: 'segment' });
  }

  moveNailiToIndoorAnchor() {
    if (this.phase !== 'nailiReturningB') return;
    const naili = this.state.naili;
    this.phase = 'nailiReturningIndoor';
    this.actorPhases.naili = 'returningIndoor';
    naili.speed = this.doorSpeed(naili);
    this.setOcclusion(naili, null);
    this.navigateNormal(naili, this.calibration.nailiReturnAnchor || this.calibration.point1,
      () => this.finishNailiReturn(), { exactTarget: true, facing: 'segment' });
  }

  finishNailiReturn() {
    if (this.phase !== 'nailiReturningIndoor') return;
    this.stopActor(this.state.naili);
    this.setOcclusion(this.state.naili, null);
