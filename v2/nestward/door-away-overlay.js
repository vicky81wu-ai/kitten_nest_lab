...
const HUBBY_SOURCES = [
  './assets/characters/hubby-idle.png',
  './assets/characters/hubby-walk-1.png',
  './assets/characters/hubby-walk-2.png',
  './assets/characters/hubby-walk-3.png',
  './assets/characters/hubby-walk-4.png'
];
...
async function loadImage(source) {
  ...
  if (typeof image.decode === 'function') {
    try { await image.decode(); return image; } catch { /* onload fallback */ }
  }
  if (image.complete && image.naturalWidth) return image;
  return new Promise((resolve, reject) => {
    image.onload = () => resolve(image);
    image.onerror = () => reject(new Error(`Door-away actor asset failed: ${source}`));
  });
}

export class DoorAwayOverlay {
  ...
  async preload() {
    this.images = await Promise.all(HUBBY_SOURCES.map(loadImage));
    this.ready = true;
    this.resize();
  }
  ...
  render(state, time, mode) {
    ...
    const sprite = hubby.walking
      ? this.images[1 + (Math.floor(hubby.step * .72) % 4)]
      : this.images[0];
