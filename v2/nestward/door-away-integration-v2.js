...
import { DoorAwayOverlayV2 } from './door-away-overlay-v2.js';
...
function syncTestUI() {
  ...
  const ready = calibrationReady && overlayReady && Boolean(stateRef);
  start.disabled = !ready || phase !== 'idle';
  recall.disabled = !ready || phase !== 'outside';
  status.textContent = calibrationError
    ? '门框参数失败'
    : !calibrationReady
      ? '读取验收参数…'
      : !overlayReady
...
function ensureOverlay(renderer) {
  ...
  if (!overlay) {
    overlay = new DoorAwayOverlayV2(document.querySelector('#world'), renderer);
    if (masks) overlay.setMasks(masks);
  }
  if (masks) overlay.setMasks(masks);
  const nextReady = overlay.ready;
  if (nextReady !== overlayReady) {
    overlayReady = nextReady;
    syncTestUI();
  }
}
...
function exactPath(actor, points, afterMove = null) {
  ...
  if (target) {
    ...
    if (Math.abs(dx) > 3) actor.dir = dx > 0 ? 1 : -1;
  }
...
function startAway() {
  if (!calibrationReady || !overlayReady || !stateRef || !planner?.ready) {
    hint(calibrationError ? '门框参数构建失败。' : '门框参数还没准备好。', 2400);
    return;
  }
...
function onDoorPointerDown(event) {
...
function onDoorPointerUp(event) {
...
function onDoorPointerCancel(event) {
  if (blockedPointerId === event.pointerId) blockedPointerId = null;
}

function installDoorGuard() {
  if (doorGuardInstalled) return;
  const canvas = document.querySelector('#world');
  if (!canvas) return;
  canvas.addEventListener('pointerdown', onDoorPointerDown, true);
  canvas.addEventListener('pointerup', onDoorPointerUp, true);
  canvas.addEventListener('pointercancel', onDoorPointerCancel, true);
  doorGuardInstalled = true;
}

function uninstallDoorGuard() {
  if (!doorGuardInstalled) return;
  const canvas = document.querySelector('#world');
  if (!canvas) return;
  canvas.removeEventListener('pointerdown', onDoorPointerDown, true);
  canvas.removeEventListener('pointerup', onDoorPointerUp, true);
  canvas.removeEventListener('pointercancel', onDoorPointerCancel, true);
  doorGuardInstalled = false;
  blockedPointerId = null;
}
...
async function prepareCalibration() {
  ...
  if (!calibration) {
    ...
    calibrationError = new Error('accepted Door Lab snapshot not found on this origin');
  ...
  try {
    ...
    calibrationError = calibrationReady ? null : new Error('temporary walk zone has no connected component');
...
export function installDoorAwayIntegrationV2() {
  makeTestUI();
  prepareCalibration();

  WorldRenderer.prototype.render = function patchedDoorAwayRenderV2(state, time) {
    stateRef = state;
    rendererRef = this;
    ensureOverlay(this);
  ...
  WorldRenderer.prototype.actorScreenBounds = function patchedDoorAwayBoundsV2(state, actor) {
