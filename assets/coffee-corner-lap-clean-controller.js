(function(){
  var VERSION = 'coffee-corner-lap-clean-controller-20260614-1';
  ...
  function ensureStyle(){
    ...
    style.textContent = [
      ...
      '.lapCleanEnterHot,.lapCleanBackHot{position:absolute;border:0;padding:0;background:transparent;border-radius:22px;z-index:37;pointer-events:auto;color:transparent;font-size:0}',
  ...
  function ensureHotspots(){
    ...
    if(!r) return false;
    if(!enterHot){
      enterHot = makeHot('lapCleanEnterHot', '坐腿上近景 clean test', enterLap);
      r.appendChild(enterHot);
    }
    if(!backHot){
      backHot = makeHot('lapCleanBackHot', '返回咖啡角 clean test', backMain);
  ...
  function placeHot(hot, card){
    ...
    var w = box.width * card.width;
    var h = box.height * card.height;
  ...
  function reinstallSteam(){
    ...
    if(steam && !steam.querySelector('svg')) steam.removeAttribute('data-steam-svg');
    if(window.KittenNestCoffeeSteam && typeof window.KittenNestCoffeeSteam.install === 'function'){
  ...
  function clearVisual(){
    ...
    if(overlay){
      ...
      overlay.style.transform = 'translateZ(0) scale(1.018) translateY(6px)';
      overlay.style.filter = 'blur(1.2px)';
    }
  }

  function swapBg(src, done){
    var image = bg();
    if(!image){ done && done(); return; }
    var settled = false;
    function finish(){
      if(settled) return;
      settled = true;
  ...
  function run(kind){
    ...
    clearVisual();
    updateHotspots();
    document.body.classList.add('lapCleanTransitioning');
      ...
      requestAnimationFrame(function(){
        requestAnimationFrame(function(){
          if(kind === 'enter'){
            ...
            image.style.transform = 'translateZ(0) scale(1.042) translateY(6px)';
            image.style.filter = 'blur(.9px)';
            overlay.style.opacity = '1';
            overlay.style.transform = 'translateZ(0) scale(1) translateY(0)';
            overlay.style.filter = 'blur(0)';
          }else{
            image.style.opacity = '0';
            image.style.transform = 'translateZ(0) scale(1.018) translateY(-4px)';
            image.style.filter = 'blur(1.1px)';
            overlay.style.opacity = '1';
            overlay.style.transform = 'translateZ(0) scale(1) translateY(0)';
            overlay.style.filter = 'blur(0)';
          }
  ...
  function leavingGuard(e){
    var t = e && e.target;
    if(!t || !t.closest || !t.closest('.toHomeHot')) return;
    var r = room();
    document.body.classList.add('leavingCoffeeCorner');
    if(r) r.classList.add('leavingCoffeeCorner');
    setTimeout(function(){
      document.body.classList.remove('leavingCoffeeCorner');
      if(r) r.classList.remove('leavingCoffeeCorner');
    }, 900);
  }

  function start(options){
    options = options || {};
    if(options.debug) document.body.classList.add('lapCleanDebug');
    ensureStyle();
    ensureOverlay();
    ensureHotspots();
    currentMainSrc();
    ...
    if(!installed){
      ...
      window.addEventListener('pageshow', function(){ setTimeout(updateHotspots, 80); setTimeout(reinstallSteam, 120); });
      document.addEventListener('visibilitychange', function(){ if(!document.hidden){ setTimeout(updateHotspots, 80); setTimeout(reinstallSteam, 120); } });
      document.addEventListener('click', function(){ setTimeout(updateHotspots, 80); }, true);
      ...
      setTimeout(updateHotspots, 200);
      setTimeout(updateHotspots, 800);
      setTimeout(reinstallSteam, 1000);
    }
  }
  ...
  function stop(){
    ...
    backHot = null;
    clearVisual();
    document.body.classList.remove('coffeeLapVariant','lapCleanDebug');
    mode = 'main';
    lock = false;
  }

  window.KittenNestLapClean = {
    version: VERSION,
    start: start,
    stop: stop,
    enterLap: enterLap,
