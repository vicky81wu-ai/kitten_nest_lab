(function(){
  var VERSION = 'scene-manifest-isolation-20260616-promoted-1';
  ...
  async function load(){
    ensureDebugStyle();
    try{
      var res = await fetch('/data/scene-manifest.v1.json?v=20260616-promoted-1', { cache:'no-store' });
  ...
  window.KittenNestSceneManifestIsolation = { version: VERSION, reconcile: reconcile, currentScene: currentScene, homeSceneId: homeSceneId };
  window.KittenNestSceneManifestIsolationTest = window.KittenNestSceneManifestIsolation;
