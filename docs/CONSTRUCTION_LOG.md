...
## 2026-08-13 powder-notebook publish deduplication

The notebook panel already used save-immediately semantics, so its current page was normally present in the permanent archive. The MCP, `/api/set-state`, and `/write` publish paths still carried an older copy-on-next-publish algorithm and could prepend that same current page again. All three paths now share one notebook publisher: the newly published page is archived immediately; an identical archived page is retained in place without losing its id, author, timestamp, or favorite state; MCP stamps Alex and browser publishing stamps Vicky. No live note or Supabase value was changed while applying the fix. Manifest validation is clean and 113/113 automated checks pass.
