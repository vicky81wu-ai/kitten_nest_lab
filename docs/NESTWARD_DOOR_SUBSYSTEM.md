...
7. Naili clears the threshold to the fixed open-floor return anchor, then Hubby follows two seconds later.
...
The full Door Away calibration export from Vicky's phone Notes is preserved verbatim at `v2/nestward/assets/door/door-away-calibration.v1.json` (SHA-256 `ea34e79cb8466be314f0b21f9295acaff157359579f2e741d8d8daab133af3f3`). It has 152 A adjustment strokes, 44 B adjustment strokes, and 44 walk strokes. The build-only script `scripts/bake-door-occlusion.mjs` produced the matching `1536 × 1024` static alpha PNGs.

Vicky visually accepted the baked masks on phone on 2026-08-20. `door-occlusion-manifest.v1.json` is therefore `accepted-baked`, with formal `maskA`, `maskB`, and `walk` entries. Do not redraw, simplify, or infer replacement contours.

The temporary upper-right controls intentionally show only semi-transparent `出去` and `叫回来` buttons while the future wall-lamp hotspot remains undecided. They have no QA label or construction-status text.
...
- The possible wall-lamp secret trigger is undecided; the retained two-button control is the temporary trigger.
