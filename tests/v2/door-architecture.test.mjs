...
test('production Door runtime owns no live-wire solver or browser-local accepted calibration', async () => {
  ...
  assert.equal(manifest.status, 'accepted-baked');
  assert.equal(manifest.activationPolicy, 'accepted-static-assets');
  assert.deepEqual(manifest.assets, {
    maskA: './door-mask-a.v1.png',
    maskB: './door-mask-b.v1.png',
    walk: './door-walk.v1.png'
  });
});

test('the retained Door Away controls contain only the two requested actions', async () => {
  const runtime = await read('nestward.js');
  assert.match(runtime, /doorAwayControls/);
  assert.match(runtime, /<button data-away-start type="button">出去<\/button><button data-away-recall type="button">叫回来<\/button>/);
  assert.doesNotMatch(runtime, /候选门框|检查门框资产|doorCandidate|doorAwayTest/);
});
