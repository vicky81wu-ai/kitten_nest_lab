...
test('the checked-in Door manifest activates Vicky’s phone-accepted baked assets by default', async () => {
  ...
  assert.equal(result.manifest.status, 'accepted-baked');
  assert.equal(result.candidate, false);
  assert.equal(result.activation, 'accepted');
  assert.deepEqual(decoded, [
    'https://example.test/door/door-mask-a.v1.png',
    'https://example.test/door/door-mask-b.v1.png',
    'https://example.test/door/door-walk.v1.png'
  ]);
});
